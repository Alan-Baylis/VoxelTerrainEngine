...
float3 bendDir2 = normalize(float3(worldPos.x,worldPos.y,worldPos.z)- float3(_Obstacle2.x,_Obstacle2.y,_Obstacle2.z));//direction of obstacle bend
float3 bendDir3 = normalize(float3(worldPos.x,worldPos.y,worldPos.z)- float3(_Obstacle3.x,_Obstacle3.y,_Obstacle3.z));//direction of obstacle bend

float distLimit = 0.35;// how far awaydoes obstacle reach
float distLimit2 = 0.75;// how far awaydoes obstacle reach
...
float distMulti2 = (distLimit-min(distLimit,distance(float3(worldPos.x,worldPos.y,worldPos.z),float3(_Obstacle2.x,_Obstacle2.y,_Obstacle2.z)))); //distance falloff
float distMulti3 = (distLimit2-min(distLimit2,distance(float3(worldPos.x,worldPos.y,worldPos.z),float3(_Obstacle3.x,_Obstacle3.y,_Obstacle3.z)))); //distance falloff
	...
	//distort.xyz += mul ((float3x3)_World2Object, waveMove).xyz;
	distort.y += float3(0,5,0)* bendDir2.y*distMulti2*v.normal.y*8*(v.color.a); 
	distort.xz += bendDir2.xz*distMulti2*v.normal.xz*2*(v.color.a);
	
	distort.y += float3(0,5,0)* bendDir3.y*distMulti3*v.normal.y*8*(v.color.a); 
	distort.xz += bendDir3.xz*distMulti3*v.normal.xz*2*(v.color.a);
